...
void DeleteFirst(PPNODE Head)
{
    ...
    if(*Head == NULL)   //case 1            
    {
        return; //karan delete karnyasathi kahi nahi ye
    }
    else if((*Head)->next == NULL)  //case 2
    {
        free(*Head);        //base address deto
        *Head = NULL;       //pahila porga gela asa sangava lagta first pointer la
    }                       // when we call deletefirst function for case 2 after successful deletion it becomes case 1
    else    //case 3
    {
        *Head = (*Head) -> next;
        free(Temp);
